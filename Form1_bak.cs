...
namespace trail01
{
    ...
    public partial class Form1 : Form
    {
        ...
        public object ComPorts { get => comPorts; set => comPorts = value; }
        ...
        private List<string> GetUSBDevices()
        {
            ...
            foreach (ManagementObject o in coll)
            {
                ...
                {
                    Console.WriteLine("-----------------------------------");
                    Console.WriteLine(o["Availability"]);
                    Console.WriteLine((string)o["Name"]);
                    //Console.WriteLine((string)o["Caption"]);
                    //Console.WriteLine((string)o["Description"]);

                    // extract COM Port
                    string device = (string)o["DeviceID"];
                    string pattern = "(.*PID_6001\\+)";
                    sTmp = Regex.Replace(device, pattern, String.Empty);
                    string port = sTmp.Substring(0,8);
                    Console.WriteLine(port);
                    //PortList.Record(port); 

                    Console.WriteLine((string)o["DeviceID"]);
                    Console.WriteLine((string)o["PNPDeviceID"]);
                    Console.WriteLine("VCP ComPort " + GetVCP_COMPort(o));
                    retList.Add(GetVCP_COMPort(o));

                    // append to list 
                    string[] arr = new string[4];
                    ListViewItem itm;
                    arr[0] = port;
                    arr[1] = GetVCP_COMPort(o);
